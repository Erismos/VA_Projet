"""
ÉTAPE 3 — Visualisation avancée (bonus)
========================================
Ce script relit le fichier de résultats MOT et la vidéo/images
pour produire une visualisation soignée avec :
  - Boîtes colorées par ID
  - Label ID + confiance
  - Trajectoire en fondu (fade progressif)
  - Compteur d'objets actifs en haut à gauche

Utile pour le rapport final et la démo de P5.
"""
...
TRAIL_LEN  = 40   # longueur de la traînée (en frames)
...
def get_color(track_id):
    ...
    # On s'assure que la couleur est assez vive (pas trop sombre)
    r = max(r, 80); g = max(g, 80); b = max(b, 80)
    return (b, g, r)


def draw_dashed_box(img, x1, y1, x2, y2, color, thickness=2, dash_len=10):
    """Dessine une boîte en pointillés (style plus visible)."""
...
# Historique des positions par ID
trail = {}   # {id: [(cx, cy), ...]}
...
for frame_idx, img_path in enumerate(image_paths):
    ...
    # Récupère les détections de cette frame
    ...
    for _, row in dets.iterrows():
        ...
        # Dessin de la traînée avec effet de fondu
        pts = trail[tid]
    ...
    # Compteur d'objets actifs (overlay en haut à gauche)
